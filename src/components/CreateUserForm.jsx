...
function SimpleRegistrationForm() {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_SERVER}/api/users`, userData);

      console.log('hello', response.data);
      alert('User created');
      // navigator('/')
      navigator(`/user/${response.data.user._id}`)
    } catch (error) {
    ...
    return (
       <Card style={{minWidth: '384px'}} className='items-center' color="transparent" shadow={false}>
        ...
        <Typography color="gray" className="mt-1 font-normal">
          Nice to meet you! Enter User details to create.
        </Typography>
        <form onSubmit={handleCreateUserSubmit} className="mt-8 mb-2 w-80 max-w-screen-lg sm:w-96">
